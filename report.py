...
<style>
:root{--surface:#f6f5f2;--card:#ffffff;--ink:#1a1a1a;--ink2:#5a5a57;--ink3:#8c8b86;--line:#e6e4df;--tile:#ecebe7;--warn:#b4531a}
...
.short{display:grid;grid-template-columns:repeat(5,1fr);gap:8px}
.short .s{background:var(--surface);border:1px solid var(--line);border-radius:10px;padding:10px 6px;text-align:center;cursor:pointer;font-size:12px}
...
<main>
  <section>
    ...
    <div class="legend"><span>less likely</span><span class="bar"></span><span>more likely</span><span style="margin-left:14px">× = mismatch flagged</span></div>
  ...
  <aside>
    ...
    <div class="card"><h2>High probability of interviewing</h2><div class="short" id="short"></div></div>
  </aside>
</main>
...
R.forEach(r => {
  ...
  t.className = "tile" + (r.mismatch ? " mm" : "");
...
$("#short").innerHTML = R.slice(0,5).map(r => `<div class="s" data-id="${r.id}" onclick="select('${r.id}')">${logo(r)}<b>${pct(r.chance)}%</b><span>${r.name}</span></div>`).join("");
